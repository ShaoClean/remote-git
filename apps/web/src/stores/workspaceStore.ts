...
const defaults: Preferences = {
  treeOpen: true,
  collapsedConnectionIds: [], connectionOrder: [], repositoryOrderByConnection: {},
};
...
function readPreferences(value: unknown): Preferences {
  ...
  return {
    ...
    repositoryOrderByConnection: Object.fromEntries(Object.entries(record(saved.repositoryOrderByConnection))
      .filter(([, ids]) => Array.isArray(ids)).map(([id, ids]) => [id, stringIds(ids)])),
  };
}
...
export const useWorkspaceStore = create<WorkspaceState>()(persist((set, get) => ({
  ...
  reconcileRepositories: (repositories) => set((state) => {
    const groups = new Map<string, string[]>();
    for (const repo of repositories) {
      if (state.validatedConnectionIds && !state.validatedConnectionIds.includes(repo.connectionId)) continue;
      const ids = groups.get(repo.connectionId) || [];
      ids.push(repo.id);
      groups.set(repo.connectionId, ids);
    }
    return {
      repositoryOrderByConnection: Object.fromEntries([...groups].map(([id, ids]) =>
        [id, orderedIds(ids, state.repositoryOrderByConnection[id])])),
    };
  }),
  reconcileConnections: (ids) => set((state) => ({
    ...
    repositoryOrderByConnection: Object.fromEntries(Object.entries(state.repositoryOrderByConnection)
      .filter(([id]) => ids.includes(id))),
  })),
  addConnection: (id) => set((state) => ({
    connectionOrder: orderedIds([...state.connectionOrder, id]),
    validatedConnectionIds: state.validatedConnectionIds ? orderedIds([...state.validatedConnectionIds, id]) : null,
  })),
  removeConnection: (id) => set((state) => ({
    connectionOrder: state.connectionOrder.filter((savedId) => savedId !== id),
    validatedConnectionIds: state.validatedConnectionIds?.filter((savedId) => savedId !== id) ?? null,
  ...
  addRepository: (repo) => set((state) => ({
    repositoryOrderByConnection: {
      ...
      [repo.connectionId]: orderedIds([...(state.repositoryOrderByConnection[repo.connectionId] || []), repo.id]),
    },
  })),
  removeRepository: (id) => set((state) => ({
    repositoryOrderByConnection: Object.fromEntries(Object.entries(state.repositoryOrderByConnection)
      .map(([connectionId, ids]) => [connectionId, ids.filter((savedId) => savedId !== id)])),
  })),
  setConnectionCollapsed: (id, collapsed) => set((state) => ({
    collapsedConnectionIds: collapsed ? stringIds([...state.collapsedConnectionIds, id])
  ...
  moveTreeItem: (source, target, placement) => {
    ...
    const state = get();
    const ids = source.kind === 'connection' ? state.connectionOrder : state.repositoryOrderByConnection[source.connectionId] || [];
    ...
    else set({ repositoryOrderByConnection: { ...state.repositoryOrderByConnection, [source.connectionId]: next } });
    return true;
  },
  ...
  partialize: ({ treeOpen, collapsedConnectionIds, connectionOrder, repositoryOrderByConnection }) =>
    ({ treeOpen, collapsedConnectionIds, connectionOrder, repositoryOrderByConnection }),

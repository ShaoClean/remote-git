...
const isMetaLine = (line: string) => line.startsWith('diff ') || line.startsWith('index ') || line.startsWith('@@') || isFileHeader(line) || line.startsWith('old mode ') || line.startsWith('new mode ') || line.startsWith('new file mode ') || line.startsWith('deleted file mode ') || line.startsWith('similarity index ') || line.startsWith('rename from ') || line.startsWith('rename to ') || line.startsWith('copy from ') || line.startsWith('copy to ') || line.startsWith('Binary files ') || line.startsWith('\\ No newline');
...
function getSplitDiffRows(diff: string): SplitDiffRow[] {
  ...
  for (let index = 0; index < lines.length;) {
    ...
    if (isAddedLine(line)) {
      ...
      added.forEach((value) => rows.push({ left: '', right: value, leftKind: 'empty', rightKind: 'add' }));
      continue;
    }
...
export function DiffViewer({ oldCode = '', newCode = '', diff, title, splitView = false, onClose, loading = false, error }: Props) {
  ...
  const renderUnifiedDiff = (value: string) => {
    ...
    return (
      <pre>{lines.map((line, index) => {
        ...
        return <span className={className} key={`${index}-${line}`}>{line}{index < lines.length - 1 ? '\n' : ''}</span>;
      })}</pre>
    );
  };
  ...
  const renderSplitDiff = (value: string) => (
    <div className="diff-split-view" aria-label="分栏差异">
      {getSplitDiffRows(value).map((row, index) => row.meta !== undefined ? (
        <div className="diff-split-row diff-split-row--meta" key={`${index}-${row.meta}`}><span>{row.meta}</span></div>
      ) : (
  ...
  const renderDiffContent = () => {
    ...
    if (error) return <div className="diff-error"><strong>无法加载差异</strong><span>{error}</span></div>;
    if (!hasDiff) return <div className="diff-empty">请选择改动文件或提交以查看差异。</div>;
    if (!diff) return <ReactDiffViewer oldValue={oldCode} newValue={newCode} splitView={mode === 'split'} compareMethod={DiffMethod.WORDS} leftTitle="原始版本" rightTitle="修改后" />;
  ...
  return (
    <>
      <div className="diff-shell">
        <div className="diff-shell__header">
          ...
          <div className="diff-toolbar">
            ...
            <Button type="text" size="small" icon={<ExpandOutlined />} aria-label="放大查看差异" title="放大查看差异" disabled={loading || Boolean(error) || !hasDiff} onClick={() => setZoomed(true)}>放大</Button>
      ...
      {zoomed && <Modal className="diff-zoom-modal" open onCancel={() => setZoomed(false)} footer={null} title={title || '差异预览'} width="calc(100vw - 48px)">
        <div className="diff-zoom-modal__body" ref={zoomBodyRef}>{renderDiffContent()}</div>
      </Modal>}
    </>
  );
}

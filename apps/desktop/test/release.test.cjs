...
async function fixture(t) {
  ...
  for (const target of release.targets) {
    for (const file of release.artifacts(target, '1.2.3')) await writeFile(path.join(directory, file), 'installer test fixture');
  }
  for (const [name, target] of [['latest.yml', 'win-x64'], ['latest-linux.yml', 'linux-x64']]) {
    const url = release.artifacts(target, '1.2.3')[0];
...
test('complete releases produce checksums for both macOS architectures', async (t) => {
